...
The Cloudinary upload path is identical to posts - images go through
the CloudinaryField on save(), videos go through
cloudinary.uploader.upload(resource_type="video") and we store the
returned public_id. Validation reuses apps.posts.views._validate_upload
so the size/MIME rules stay in one place.
"""
...
# ─────────────────────────────────────────────────────────────
# MEDIA UPLOAD  - mirrors upload_post_media
# ─────────────────────────────────────────────────────────────
...
def upload_highlight_media(request):
    ...
    if media_type == "video":
        ...
        item = HighlightItem.objects.create(
            ...
            media_type="video", order=current_count, duration=duration)
    else:
        item = HighlightItem.objects.create(
            highlight=highlight, file=file, media_type="image",
            order=current_count, duration=duration)
...
@api_view(["DELETE"])
@permission_classes([permissions.IsAuthenticated])
def delete_highlight_item(request, pk):
    """DELETE /api/highlights/items/<pk>/"""
    try:
        item = HighlightItem.objects.select_related("highlight").get(pk=pk)
